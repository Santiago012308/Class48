...
win=createSprite(300,300,10,10);
win.addImage(winImg);
win.scale=.5;
win.visible=false;
win.velocityX=3;
win.velocityY=-2;
exit=createSprite(451,590,70,18);
exit.shapeColor="red";
}
function draw() {
  win.bounceOff(edges);
...
if(lukas.isTouching(coin5)){
 ...
 if(lukas.y>560&&lukas.x>450&&score!==25){
  textSize(10)
  text("Collect all the coins before reaching the exit to win!",90,54);
 }
  if(score===25){
    exit.destroy();
  }
  if(lukas.y>594&&score===25){
    gameState=1;
  }
}
if(gameState===1){
